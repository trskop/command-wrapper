...
executeCommandWith f appNames globalConfig subcommand arguments =
  ...
  where
    ...
    getParams prefix command = do
        config <- getSubcommandConfigPath' appNames configPaths prefix command
...
getSubcommandConfigPath'
  ...
  command = do
    ...
    pure if
      | haveLocalConfig, Just file <- localConfig ->
            file

      | haveUserConfig ->
            userConfig

      | haveSystemConfig, Just file <- systemConfig ->
            file

      | otherwise ->
            fromMaybe userConfig localConfig
