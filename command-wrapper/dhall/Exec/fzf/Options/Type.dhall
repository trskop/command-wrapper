...
let FzfOptions =
      ...
      -- `None Bool` value means that neither is passed and default value is
      -- used. `Some True` means "Yes", use extended-search mode.
      ...
      -- `None Text` means that default delimiter is used, i.e. no
      -- `--delimiter=REGEX` is passed.
      , delimiter : Optional Text

      -- Sort the result?  `None Bool` mans that neither `--sort` nor
      -- `--no-sort` will be passed to `fzf`.  `Some True` means "Yes", sort
      -- the result.
      , sortResult : Optional Bool

      -- Reverse order of the input?  `Some True` means "Yes", reverse order of
      -- the input.
      , reverseInputOrder : Optional Bool

      -- Empty list means that default value is used, i.e. no command line
      -- option is passed.
      ...
      -- *   `False`, leave mouse enabled.  There is no `--mouse` option,
      --     therefore, we just don't pass any option to `fzf`.
      , noMouse : Bool
      ...
      -- *  `TopOfTheScreenPromptAtTheBottom` -- display from the top of the
      --    screen, prompt at the bottom.
      , layout :
            Optional
              < BottomOfTheScreen
              | TopOfTheScreen
      ...
      -- Use Unicode box drawing characters?  `Some True` means use Unicode,
      -- and `Some False` means use ASCII.
